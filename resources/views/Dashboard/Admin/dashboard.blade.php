...
@section('page-header')
				...
				<div class="breadcrumb-header justify-content-between">
					<div class="left-content">
						<div>Let's say this boy actually 
						  <h2 class="main-content-title tx-24 mg-b-1 mg-b-lg-1">Hi, welcome <u><b>{{trans('Dashboard/login_trans.admin')}}</b></u></h2>
...
@section('content')
							@php
							$q_users=$userCounts = DB::table('user_counts')
									->where('name', 'users')
									->orderBy('user_counts.id', 'DESC')
									->limit(7);
							

							$data=$q_users->pluck('count'); //Get the Count Column from the table
							$Conditional_count = $data->count(); //count the number of rows
							$Current_Users_Num = $data->first();	//get the last entered data based on the id.

							$q_admins=DB::table('user_counts')
									->where('name', 'admins')
									->orderBy('user_counts.id', 'DESC')
									->limit(7);
							$data_admins=$q_admins->pluck('count'); //Get the Count Column from the table
							$Conditional_count_admins = $data_admins->count(); //count the number of rows
							$Current_Users_Num_admins = $data_admins->first();	//get the last entered data based on the id.
				...
				<div class="row row-sm">
					<div class="col-xl-3 col-lg-6 col-md-6 col-xm-12">
						<div class="card overflow-hidden sales-card bg-primary-gradient">
							<div class="pl-3 pt-3 pr-3 pb-2 pt-0">
								<div class="">
									<h6 class="mb-3 tx-16 text-white"><b>{{trans('Dashboard/login_trans.user')}}</b></h6>
								</div>
								<div class="pb-0 mt-0">
									<div class="d-flex">
										<div class="">
										...
										<span class="float-right my-auto mr-auto">
											<i class="fas fa-arrow-circle-{{$data->first()-$data->last()<0 ? 'down': 'up'}} text-white"></i>
										<span class="text-white op-7">{{$data->first()-$data->last()<0 ?($data->first()-$data->last()) : '+'.($data->first()-$data->last())}}</span>
										</span>
									</div>
								</div>
							</div>
							<!-- print_r(DB::table("user_counts")->where('name','=','Users')->pluck('count')->get()) -->
					...
					<div class="col-xl-3 col-lg-6 col-md-6 col-xm-12">
						<div class="card overflow-hidden sales-card bg-danger-gradient">
							<div class="pl-3 pt-3 pr-3 pb-2 pt-0">
								<div class="">
								<h6 class="mb-3 tx-16 text-white"><b>{{trans('Dashboard/login_trans.admin')}}</b></h6>
								</div>
								<div class="pb-0 mt-0">
									<div class="d-flex">
										<div class="">
										...
										<span class="float-right my-auto mr-auto">
											<i class="fas fa-arrow-circle-{{$data_admins->first()-$data_admins->last()<0 ? 'down': 'up'}} text-white"></i>
											<span class="text-white op-7"> {{$data_admins->first()-$data_admins->last()<0 ? ($data_admins->first()-$data_admins->last()) : '+'.($data_admins->first()-$data_admins->last())}}</span>
										</span>
									</div>
								</div>
							</div>
